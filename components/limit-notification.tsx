...
export function LimitNotification({ remaining, isPro, onUpgrade, className = '', compact = false }: LimitNotificationProps) {
  if (isPro) {
    if (remaining === Infinity) {
      if (compact) {
        return (
          <div className={`bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200/50 rounded-xl px-3 py-2 shadow-sm ${className}`}>
      ...
      if (compact) {
        return (
          <div className={`bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200/50 rounded-xl px-3 py-2 shadow-sm ${className}`}>
            <div className="flex items-center space-x-2">
  ...
  if (compact) {
    return (
      <div className={`bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-xl px-3 py-2 shadow-sm ${className}`}>
        <div className="flex items-center space-x-2">
          <Sparkles className="h-4 w-4 text-blue-600" />
          <span className="text-sm font-medium text-gray-700">{remaining}/3</span>
        </div>
      </div>
    );
  }
